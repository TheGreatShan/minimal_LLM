...
public class PromptConstructLatin : IPromptConstruct<Task<List<string>>>
{
    ...
    public PromptConstructLatin(PromptParams promptParams, Config config,
        Illm<IAsyncEnumerable<string>, string, LlamaInstance> llamaSharpLlm)
    {
        ...
        var text = string.Join(" ",
            File.ReadAllText(promptParams.ContextFile).Split(delimiterChars, StringSplitOptions.TrimEntries));
    ...
    public async Task<List<string>> Construct(string prompt)
    {
        ...
        var initPrompt =
            "Transcript of a dialog, where the User interacts with an assistant named TScript.\nUser: Hello, TScript.\nPypy:Hello. How may I help you today with you questions?\n";
        ...
        for (int i = 0; i < length; i++)
        {
            for (int j = i + 1; j < length; j++)
            {
                ...
                bool iBetter = false;
                string question =
                    $"Answer only with yes or no and nothing else. Is text \"i\" more relevant than text \"j\" to the question \"{prompt}\"."
